...
object DNASeqAnalyzer {
  ...
  def bwaRun(chunkFile: String, config: Configuration): Array[(Int, SAMRecord)] = {
    ...
    HDFSHelper.mkdirs(bwaLogFolder)

    // Download chunk to temporary folder
    HDFSHelper.copyToLocalFile(s"$inputFolder/$chunkFile", tmpFolder)

    val cmd = Seq(s"$toolsFolder/bwa", "mem", s"$refFolder/$RefFileName", "-p",
      "-t", numOfThreads, s"$tmpFolder/$chunkFile")

    // Create the empty file in which we'll store the log
    HDFSHelper.createEmptyFile(bwaLogFileName)

    HDFSHelper.appendLineToFile(bwaLogFileName, s"$getTimeStamp BWA mem started: ${cmd.mkString(" ")}")
    ...
    HDFSHelper.appendLineToFile(bwaLogFileName, s"$getTimeStamp BWA mem completed. Number of key value pairs: ${kvPairs.length}")

    // Delete the temporary files
    tmpOutputFile.delete
    new File(s"$tmpFolder/$chunkFile").delete
    new File(s"$tmpFolder/.$chunkFile.crc").delete
  ...
  def variantCall(chrRegion: Int, samRecordsSorted: Array[SAMRecord], config: Configuration): Array[(Int, (Int, String))] = {
    ...
    HDFSHelper.mkdirs(vcLogFolder)

    // Create the empty output file
    HDFSHelper.createEmptyFile(vcLogFileName)

    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp Writing SAM records to BAM file")
    ...
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd1.mkString(" ")}")
    cmd1.!
    ...
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd2.mkString(" ")}")
    cmd2.!
    ...
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd3.mkString(" ")}")
    cmd3.!

    val cmd4 = Seq("java", MemString, "-jar", s"$toolsFolder/BuildBamIndex.jar",
      s"INPUT=$tmpFolder/region$chrRegion.bam")
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd4.mkString(" ")}")
    cmd4.!
    ...
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd5.mkString(" ")}")
    ...
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd6.mkString(" ")}")
    cmd6.!
    ...
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd7.mkString(" ")}")
    cmd7.!
    ...
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd8.mkString(" ")}")
    cmd8.!
    ...
    val cmd9 = Seq("java", MemString, "-jar", s"$toolsFolder/GenomeAnalysisTK.jar",
      ...
      s"$tmpFolder/region$chrRegion.table", "-L", s"$tmpFolder/bed$chrRegion.bed")
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd9.mkString(" ")}")
    cmd9.!
    ...
    val cmd10 = Seq("java", MemString, "-jar", s"$toolsFolder/GenomeAnalysisTK.jar", "-T",
      ...
      "--disable_auto_index_creation_and_locking_when_reading_rods")
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp ${cmd10.mkString(" ")}")
    cmd10.!
    HDFSHelper.appendLineToFile(vcLogFileName, s"$getTimeStamp Output written to VCF file")
    ...
    // Copy the result VCF file to HDFS
    HDFSHelper.copyFromLocalFile(s"$tmpFolder/region$chrRegion.vcf", outputFolder)

    // Read the output VCF file produced by the haplotype caller
    val source = Source.fromFile(s"$tmpFolder/region$chrRegion.vcf").getLines.toList

    // Delete the output VCF file from the local file system
    new File(s"$tmpFolder/region$chrRegion.vcf").delete
  ...
  def main(args: Array[String]) {
    ...
    HDFSHelper.mkdirs(outputFolder)
    HDFSHelper.mkdirs(s"$outputFolder/log/")

    // Get the list of files in the input folder
    val chunkList = HDFSHelper.listFiles(inputFolder)
    ...
    // Combine the results, sort them and write them to the output file in the local file system
    val outputFileName = s"${tmpFolder}result.vcf"
    val outputVcfWriter = new PrintWriter(new BufferedWriter(new FileWriter(outputFileName)), true)
    ...
    // Copy the combined VCF output from the local file system to HDFS
    HDFSHelper.copyFromLocalFile(outputFileName, outputFolder)
